...
**Pré-condição obrigatória:** o arquivo deve ter prefixo `url_`, `modelo: url-resumo`, `estado: finalizado` e possuir cache `{slug}-transcript.txt` em `.agents/skills/processar-url/scripts/temp/`. Sem o cache, o fluxo aborta com instrução para executar `/processar-url` primeiro.
...
3. Avalia o resumo em três eixos, usando o transcript como fonte de verdade e o template `models/url-resumo.md` como régua de estrutura:
