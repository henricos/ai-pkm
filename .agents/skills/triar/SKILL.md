...
- **Markdown:** leia o conteúdo e determine se é `tipo: url` ou `tipo: nota`.
...
|---|---|
| Apenas uma URL, sem nenhum texto | `tipo: url` |
| Uma URL + no máximo uma frase curta como rótulo/título (ex: `"Artigo sobre X"`, `"Fonte: Y"`) | `tipo: url` |
| Qualquer coisa além disso: frases de contexto, parágrafos, citações, listas, notas pessoais | `tipo: nota` |

Exemplos concretos:

```
# tipo: url — apenas URL + rótulo curto
https://exemplo.com/artigo

# tipo: url — URL com título de uma linha
Guia de fine-tuning de LLMs
https://exemplo.com/fine-tuning

# tipo: nota — tem frases de contexto, mesmo que o assunto seja "uma referência"
...
- Se o arquivo tiver mais de uma frase, ou qualquer texto que vá além de um rótulo de link, é `tipo: nota`.
- A presença de atribuição bibliográfica, cabeçalho, seções ou notas pessoais transforma automaticamente o item em `tipo: nota`.
...
### Passo 2.5: Determinação do Template (somente para `tipo: nota`)

Para cada item classificado como `tipo: nota`, determine o template adequado consultando `sistema/indices/templates.json`.

**Lógica de resolução por item:**

1. **Encaixe claro** — o conteúdo ou o nome do arquivo sinaliza claramente um único template. Defina automaticamente, sem perguntar ao usuário.
...
2. **Ambiguidade** — o item poderia encaixar em dois ou mais templates. Antes de prosseguir, questione o usuário usando `AskUserQuestion` com as opções candidatas (inclua "Nenhum — omitir campo" como última opção).

3. **Sem encaixe aparente** — nenhum template disponível parece se aplicar ao conteúdo. Questione o usuário via `AskUserQuestion` listando os templates disponíveis e incluindo "Nenhum — omitir campo" como opção. Somente se o usuário confirmar "Nenhum" o campo `template` é omitido no frontmatter.
...
- Para `tipo: url` e sidecars de binários: o campo `template` nunca é usado.
...
1. Determine o **tópico** — consulte `pkm/sistema/indices/topicos.json`. Nunca invente tópicos que não constem no mapa.
2. Determine se pertence a um **grupo** existente — consulte `pkm/sistema/indices/grupos.json` filtrado pelo tópico escolhido.
...
- **URLs (`tipo: url`):** prefixo `url_` obrigatório + padrão autor-título (ex: `url_ibm-technology-rag-vs-long-context.md`)
...
### Passo 5: Etapas Adicionais para Itens com URL (`tipo: url`)

Quando o tipo for `url`, execute estas etapas antes de montar a proposta:

#### 5.1 Estado inicial

Todo item com `tipo: url` recebe `processado: false` na triagem, independente de ter contexto ou não.
...
#### 5.3 Tópico e descrição

- **Tópico (`topico`):** Obrigatório. Deve coincidir com o tópico do destino.
- **Descrição (`descricao`):** Gere uma descrição curta e informativa, otimizada para busca.

#### 5.4 Formato de armazenamento

O `formato` é preenchido automaticamente por regra, sem perguntar ao usuário por item:
...
O nome final de todo arquivo `tipo: url` deve seguir obrigatoriamente a convenção em `docs/pkm-naming.md`: prefixo `url_` + autor-título. Ao propor o nome final, siga esta ordem de prioridade:
...
Monte um sidecar no formato `nome.extensao.md` com frontmatter conforme `docs/schemas/frontmatter-conhecimento.md`.

Regras:
- `descricao` é obrigatória e deve explicar resumidamente o conteúdo do binário.
- `topico` deve coincidir com o destino.
...
```
| # | Arquivo original | Tipo | Destino proposto | Nome final | Template | Formato |
|---|---|---|---|---|---|---|
| 1 | nota-xyz.md | nota | _saude/ | reflexoes-sobre-jejum.md | — | — |
| 2 | link-abc.md | url | _tecnologia/ | url_autor-titulo.md | — | extrato |
| 3 | diagrama-bruto.svg | nota (binário) | _tecnologia/ | arquitetura-de-agentes.svg | — | — |
```

A coluna `Template` é exibida apenas para itens `tipo: nota` e preenchida automaticamente quando há encaixe claro; caso contrário fica `—` (omitido no frontmatter). A coluna `Formato` é exibida apenas para itens `tipo: url` e preenchida automaticamente por regra: `resumo` para vídeos (YouTube, Instagram, TikTok), `extrato` para web/PDF. O usuário pode alterar qualquer valor antes de confirmar.

**Para cada item que terminará com frontmatter ou sidecar**, mostre também a proposta logo abaixo da tabela:

```yaml
# Item 2 — guia-fine-tuning-llms.md
---
descricao: "Guia prático sobre fine-tuning de LLMs com técnicas atuais"
topico: tecnologia
tipo: url
...
---
descricao: "Reflexões sobre jejum intermitente e seus efeitos no metabolismo"
topico: saude
tipo: nota
maturidade: rascunho
template: nota-conceito        # omitir se nenhum template se aplicar
...
---
descricao: "Diagrama visual da arquitetura de agentes do sistema"
topico: tecnologia
tipo: nota
data_captura: YYYY-MM-DD
---
...
Inclua `autores` e `data_publicacao` somente se identificados. Nunca use `null` ou lista vazia para campos opcionais — simplesmente omita-os.
...
**Para Markdown (`tipo: url` ou `tipo: nota`):**

```json
{
  "tipo": "markdown",
  "origem": "__inbox/nome-original.md",
  "destino": "_topico/nome-final.md",
...
{
  "tipo": "binario",
  "origem": "__inbox/arquivo.svg",
  "destino": "_topico/arquivo.svg",
  "sidecar_destino": "_topico/arquivo.svg.md",
  "sidecar_frontmatter": "descricao: \"...\"\ntopico: tecnologia\ntipo: nota\ndata_captura: 2026-03-22"
}
```
...
- **Binários em `_[topico]/...` sempre exigem sidecar após a triagem.**
- **Ação padrão para itens com URL é `extrair`.**
- **Hierarquia de classificação:** tópico primeiro, depois grupo dentro do tópico.
- **Nunca invente tópicos.** Consulte `pkm/sistema/indices/topicos.json`.
...
- **Frontmatter** segue rigorosamente `docs/schemas/frontmatter-conhecimento.md`. Campos opcionais desconhecidos são omitidos.
- **`url` é exclusivo para URLs externas e nunca aparece em `tipo: nota`.**
- **`formato` e `processado` só existem quando `tipo: url`.**
- **Sem logs** — fase 1: auditoria exclusivamente via histórico Git com `/commit-push`.
...
- `pkm/sistema/indices/grupos.json` — grupos existentes
- `pkm/sistema/indices/topicos.json` — tópicos para classificação
- `sistema/indices/templates.json` — catálogo de templates disponíveis para notas
