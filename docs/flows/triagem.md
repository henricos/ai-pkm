...
1. Determina o **tópico** (via `index/topicos.json`). Se nenhum tópico adequado, sinaliza e mantém na inbox.
2. Determina se o item pertence a um **grupo** existente (via `index/grupos.json` filtrado pelo tópico).
...
- Um item é **URL** somente se o conteúdo completo for uma URL isolada, no máximo acompanhada de uma frase curta como rótulo ou título do link (equivalente ao texto âncora de um hyperlink). Nada além disso.
...
**Tratamento de notas:** Notas recebem `estado: rascunho` no frontmatter automaticamente, sem intervenção do usuário. O campo é obrigatório e nunca deve ser omitido.

**Modelo de nota:** A triagem tenta identificar o modelo mais adequado consultando `index/models.json`. O campo `modelo` recebe o nome do arquivo de modelo sem extensão (ex: `nota-ferramenta`). A lógica de resolução é:
1. **Encaixe claro** — o conteúdo ou o nome do arquivo sinaliza claramente um único modelo (ex: "Nota sobre a ferramenta X" → `nota-ferramenta`). Defina o campo automaticamente, sem perguntar.
...
**Apresentação:** Tabela com item, tipo, destino proposto, nome final e coluna `Modelo` (preenchida por regra para todos os itens, editável). Aprovação em lote.
